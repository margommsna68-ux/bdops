    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500">{t("profit_subtitle")}</p>
        </div>
        <Button onClick={() => setShowForm(true)}>{t("profit_new_period")}</Button>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center text-center">
          <div>
            <p className="text-sm text-gray-500">{t("profit_total_exchange")}</p>
            <p className="text-2xl font-bold text-green-600">{formatCurrency(grandWithdrawal)}</p>
          </div>
          <div className="text-2xl font-bold text-gray-400">-</div>
          <div>
            <p className="text-sm text-gray-500">{t("profit_total_costs")}</p>
            <p className="text-2xl font-bold text-red-600">{formatCurrency(grandCost)}</p>
          </div>
          <div className="text-2xl font-bold text-gray-400">=</div>
          <div>
            <p className="text-sm text-gray-500">{t("profit_net")}</p>
          ...
          {splits?.map((split) => (
            <div key={split.id} className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <Badge className={split.settled ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}>
                    {split.settled ? t("settled") : t("pending")}
                  </Badge>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                <div>
                  <p className="text-sm text-gray-500">{t("profit_withdrawal")}</p>
                  <p className="text-lg font-bold text-green-600">{formatCurrency(Number(split.totalWithdrawal))}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">{t("profit_cost")}</p>
        ...
        <DialogContent className="max-w-md">
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>{t("profit_period_start")}</Label>
                <Input
                  type="date"
              ...
              <div>
                <Label>{t("profit_period_end")}</Label>
                <Input
                  type="date"
            ...
            <div>
              ...
              >
                {t("profit_add_partner")}
              </Button>
              <p className="text-xs text-gray-500 mt-1">
                {form.partners.reduce((s, p) => s + p.percentage, 0)}% — {t("profit_total_pct")}
              </p>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={createSplit.isLoading}>
                {createSplit.isLoading ? t("creating") : t("profit_create_calc")}
              </Button>
            </div>
          </form>
        </DialogContent>
